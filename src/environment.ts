...
import { skyColor, waterNear, waterFar, worldTint, sunDirection, foamColor } from './world/materials';
...
export const periods: Record<TimeOfDay,{label:string;sky:string;near:string;far:string;tint:string;foam:string;wind:number;icon:string}> = {
  ...
  sunset:{label:'夕阳',sky:'#e6bd9c',near:'#ab9e94',far:'#d7b89e',tint:'#ffd0a0',foam:'#ffe0b5',wind:.35,icon:'◒'},
...
export class Environment {
  ...
  constructor(private scene:THREE.Scene,private camera:THREE.Camera){
    ...
    this.sun=new THREE.Mesh(new THREE.SphereGeometry(1.45,24,16),new THREE.MeshBasicMaterial({color:'#efcf9c',transparent:true,opacity:0,fog:false}));this.sun.position.set(-25,17,-22);scene.add(this.sun);
  ...
  update(dt:number){
    ...
    this.sun.position.set(.58,.57,.08).unproject(this.camera);this.moon.position.set(.58,.57,.08).unproject(this.camera);
    ...
    sunDirection.value.lerp(this.period==='sunset'?new THREE.Vector3(-.9,.25,.3):this.period==='night'?new THREE.Vector3(.2,.75,-.5):new THREE.Vector3(-.45,.85,.35),blend);
    ...
    const sm=this.sun.material as THREE.MeshBasicMaterial,mm=this.moon.material as THREE.MeshBasicMaterial;
    sm.opacity+=((this.period==='sunset'?.85:0)-sm.opacity)*blend;mm.opacity+=((this.period==='night'?.9:0)-mm.opacity)*blend;
  }
}
