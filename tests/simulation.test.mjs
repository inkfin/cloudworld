...
test('terrain is finite and land stays above sea throughout the island',async()=>{const s=await world();for(let x=-30;x<=30;x+=.4)for(let z=-25;z<=25;z+=.4)if(s.radius(x,z)<=1){assert.ok(Number.isFinite(s.height(x,z)));assert.ok(s.height(x,z)>.19);}});
...
test('large frame delays are capped and reset returns to beach',async()=>{const s=await world();s.step(1,0,100,1);assert.ok(Math.abs(s.x())<.3);s.reset();assert.equal(s.x(),0);assert.equal(s.z(),18);assert.equal(s.speed(),0);});
